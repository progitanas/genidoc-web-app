// CommonJS version of genidoc-map helpers for Node tests
function extractPhones(text) {
  const phoneRegex = /(\+?\d{1,3}[\s.-]?)?(\(?\d{2,4}\)?[\s.-]?)?[\d\s.-]{6,}/g;
  const matches = (text || "").match(phoneRegex) || [];
  return Array.from(new Set(matches.map((s) => s.trim()))).slice(0, 5);
}
